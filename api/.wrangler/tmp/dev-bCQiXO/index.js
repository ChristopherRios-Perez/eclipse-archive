...
// .wrangler/tmp/bundle-nR76Kq/checked-fetch.js
...
// .wrangler/tmp/bundle-nR76Kq/strip-cf-connecting-ip-header.js
...
auth.post("/register", async (c) => {
  ...
  const user2 = { username, email, password, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
  await c.env.ECLIPSE_KV.put(`user:${email}`, JSON.stringify(user2));
...
auth.post("/login", async (c) => {
  ...
  const raw2 = await c.env.ECLIPSE_KV.get(`user:${email}`);
  if (!raw2)
  ...
  return c.json({ token, username: user2.username, email: user2.email });
});
var auth_default = auth;

// src/routes/progress.js
var progress = new Hono2();
function emailFromToken(c) {
  const auth2 = c.req.header("Authorization") ?? "";
  const token = auth2.replace("Bearer ", "");
  if (!token)
    return null;
  try {
...
__name(emailFromToken, "emailFromToken");
progress.get("/", async (c) => {
  const email = emailFromToken(c);
  if (!email)
    return c.json({ error: "Unauthorised." }, 401);
  const raw2 = await c.env.ECLIPSE_KV.get(`progress:${email}`);
...
__name(emailFromToken2, "emailFromToken");
user.get("/", async (c) => {
  const email = emailFromToken2(c);
  if (!email)
    return c.json({ error: "Unauthorised." }, 401);
  const raw2 = await c.env.ECLIPSE_KV.get(`user:${email}`);
  if (!raw2)
    return c.json({ error: "User not found." }, 404);
  const { username, email: userEmail, createdAt } = JSON.parse(raw2);
...
user.put("/", async (c) => {
  ...
  if (!raw2)
    return c.json({ error: "User not found." }, 404);
  const userData = JSON.parse(raw2);
...
// .wrangler/tmp/bundle-nR76Kq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nR76Kq/middleware-loader.entry.ts
